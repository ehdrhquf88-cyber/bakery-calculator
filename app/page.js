...
// -------------------------------------------------------------
// [RecipeCalculator 컴포넌트] (수식, 폰트 및 고유 디자인 보존)
// -------------------------------------------------------------
...
function SummaryCard({ title, children }) {
  return (
    <div className="bg-[#f7f6f3] rounded-2xl p-5 md:p-6 shadow-lg border border-white/50 relative">
      {children}
    </div>
  );
}
...
// -------------------------------------------------------------
// [RecipeDB 컴포넌트] (디자인 및 폰트 원본 유지 + 우측 상단 ✕ 삭제 버튼)
// -------------------------------------------------------------
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8">
      <div className="bg-[#f7f6f3] rounded-2xl p-5 md:p-6 shadow-lg border border-white/50">
        <h2 className="text-xl md:text-2xl font-black tracking-tighter border-b-2 border-black pb-2 mb-6 uppercase">Recipe Database</h2>
        {recipes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recipes.map(recipe => (
              <div key={recipe.id} className="bg-white/60 backdrop-blur-md rounded-xl p-4 border border-white/80 relative group hover:border-black/20 transition-all shadow-sm">
                {/* 우측 상단 ✕ 삭제 버튼 */}
                <button 
                  onClick={() => handleDeleteRecipe(recipe.id)}
                  className="absolute top-3 right-3 text-gray-400 hover:text-black font-black text-sm p-1 transition-colors leading-none"
                  title="레시피 삭제"
                >
                  ✕
                </button>
                <div className="pr-6">
                  <span className="text-[9px] font-black tracking-wider bg-black text-white px-1.5 py-0.5 rounded uppercase">{recipe.category}</span>
                  <h3 className="text-lg font-black text-gray-900 tracking-tight mt-2">{recipe.productName}</h3>
                  <div className="text-[11px] font-bold text-gray-400 mt-1 uppercase tracking-tight">
                    Ingredients: <span className="font-mono text-black">{recipe.ingredients?.length || 0}</span> items
                  </div>
                </div>
              </div>
            ))}
...
// -------------------------------------------------------------
// [TempPhDB 컴포넌트] (디자인 및 폰트 원본 유지 + 우측 상단 ✕ 삭제 버튼)
// -------------------------------------------------------------
function TempPhDB({ tempLogs, setTempLogs }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8">
      <div className="bg-[#f7f6f3] rounded-2xl p-5 md:p-6 shadow-lg border border-white/50">
        <h2 className="text-xl md:text-2xl font-black tracking-tighter border-b-2 border-black pb-2 mb-6 uppercase">Temp & pH History</h2>
        {tempLogs.length === 0 ? (
          <p className="text-center py-12 text-gray-400 font-bold uppercase text-xs tracking-widest">No logs recorded yet.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tempLogs.map(log => (
              <div key={log.id} className="bg-white/60 backdrop-blur-md rounded-xl p-4 border border-white/80 relative group hover:border-black/20 transition-all shadow-sm flex flex-col justify-between">
                {/* 우측 상단 ✕ 삭제 버튼 */}
                <button 
                ...
                <div className="pr-6">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[9px] font-black tracking-wider bg-black text-white px-1.5 py-0.5 rounded uppercase">{log.type}</span>
                    <span className="font-mono text-[10px] font-bold text-gray-400">{log.displayTime}</span>
                  </div>
                  <h3 className="text-base font-black text-gray-900 tracking-tight mt-2">{log.productName}</h3>
                  {log.memo && (
                    <p className="text-xs font-bold text-gray-600 bg-white/80 p-3 rounded-lg border border-white mt-2 line-clamp-3">
                      {log.memo}
                    </p>
                  )}
                </div>
