      ...
      <div className="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl">
        ...
        <div className="bg-gray-50 rounded-xl p-3 mb-6 text-center">
          ...
          <span className="font-black text-lg">
            {((custom ? parseFloat(custom) || 0 : selected) * 1000).toLocaleString()}g
          </span>
        </div>

        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 bg-gray-100 py-3 rounded-xl font-bold text-sm uppercase">취소</button>
          <button onClick={handlePrint} className="flex-1 bg-black text-white py-3 rounded-xl font-bold text-sm uppercase shadow-lg">
            🖨 프린트
          </button>
        </div>
      </div>
...
function RecipeCalculator({ recipes, setRecipes, tempLogs, setTempLogs }) {
  ...
  const handleDoughMultiplierChange = (value) => {
    ...
    if (isNaN(multiplier) || multiplier <= 0) { setFlourWeight(""); setTotalDough(""); return; }
    setTotalDough(Math.round(totals.baseTotalDough * multiplier));
  ...
  useEffect(() => {
    if (currentRecipe && totals.totalPercent > 0) {
      ...
      setFlourWeight(Math.round(1000));
      setTotalDough(Math.round(1000 * (totals.totalPercent / 100)));
    } else {
      setTotalDough("");
      setFlourWeight("");
      setDoughMultiplier("1");
  ...
  // 배수 적용 후 프린트
  const handlePrintWithMultiplier = (multiplier) => {
    ...
    <main className="max-w-6xl mx-auto px-4 md:px-8 text-black print:px-0 print:max-w-full">
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          ...
          .print-multiplier-label { display: block !important; }
        }
      `}} />
      ...
      {showPrintModal && (
        <PrintModal
          ...
          onPrint={handlePrintWithMultiplier}
        />
      )}
        ...
        <section className="bg-[#f7f6f3] rounded-2xl p-5 md:p-6 shadow-lg border border-white/50 order-1 print:bg-white print:shadow-none print:border-none print:p-0">
          <div className="border-b-2 border-black pb-3 mb-6 flex justify-between items-end">
            <div>
              ...
              {/* 프린트 시에만 보이는 배수 라벨 */}
              {currentRecipe && (
                <p className="hidden print-multiplier-label text-xs font-bold text-gray-500 mt-0.5">
                  밀가루 기준: {flourMultiplier || doughMultiplier || "1"}배
                  &nbsp;({Number(flourWeight).toLocaleString()}g)
                </p>
              )}
            </div>
              ...
              {currentRecipe && (
                <tfoot>
                  <tr className="border-t-2 border-black">
                    <td className="p-2 font-black text-xs uppercase">합계</td>
                    <td className="p-2 text-right font-mono font-black text-xs">{totals.totalPercent.toFixed(1)}%</td>
                    <td className="p-2 text-right font-mono font-black text-xs">
                      {flourWeight ? Number(totalDough).toLocaleString() : 0}g
                    </td>
                  </tr>
                </tfoot>
              )}
